            ...
            {wallets.map(wallet => (
              ...
              >
                ...
                <div className="relative w-12 h-12 rounded-xl bg-gradient-to-br from-purple-500/20 to-purple-600/5 flex items-center justify-center border border-purple-500/20">
                  {wallet.icon ? (
                    <img src={wallet.icon} alt={wallet.name} className="w-7 h-7 object-contain rounded-sm" />
                  ) : (
                    <img src="/logos/apt-logo.svg" alt="APT" className="w-7 h-7 object-contain rounded-sm" />
                  )}
                </div>
                <div className="relative flex-1">
                  <div className="flex items-center gap-2">
                    <h3 className="text-white font-bold text-base tracking-tight">{wallet.name}</h3>
                    <span className="text-[8px] bg-purple-500/10 text-purple-500 px-1.5 py-0.5 rounded border border-purple-500/20 font-bold uppercase">Mainnet</span>
                  </div>
                  <p className="text-gray-500 text-[11px] mt-0.5 font-medium">Aptos Wallet</p>
                </div>
